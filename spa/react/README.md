...
- Sign-in with the authorization code flow, PKCE `S256`, and `state`, as a public client with no secret, using `oidc-client-ts` through `react-oidc-context`.
...
- Node.js 22.12 or later: Vite 8 and `@vitejs/plugin-react` require `^20.19.0 || >=22.12.0`, and Node.js 20 is end of life.
...
2. Add both sign-in redirect URIs: `http://localhost:3000/callback` and `http://localhost:3000/`. The console has no separate post-logout field, and sign-out can only return to an address registered as a sign-in redirect URI, so the home address is registered too.
3. Add the allowed origin `http://localhost:3000`.
...
| Key | Value |
|---|---|
| `VITE_ALGEBRIX_ISSUER` | Your issuer, for example `https://auth.algebrix.co/realms/your-organization` |
| `VITE_ALGEBRIX_CLIENT_ID` | The application's client ID |
| `VITE_ALGEBRIX_REDIRECT_URI` | `http://localhost:3000/callback` |
| `VITE_ALGEBRIX_POST_LOGOUT_REDIRECT_URI` | `http://localhost:3000/` |

Vite exposes only keys that start with `VITE_` to the browser, and everything in them is public. Never put a secret here.
...
- **Issuer mismatch:** `oidc-client-ts` does not compare `VITE_ALGEBRIX_ISSUER` with the issuer in the discovery document, so the path form (without `/realms/`) still signs in here. Stricter libraries, such as the one in the server-side web sample, refuse to start with it. Always use the exact issuer value from the console integration view.
- **Error on the identity server's own page at sign-in, with no return to the app:** `VITE_ALGEBRIX_REDIRECT_URI` is not registered on the application exactly.
- **"Invalid redirect uri" on the identity server's page at sign-out:** `VITE_ALGEBRIX_POST_LOGOUT_REDIRECT_URI` is not registered as a sign-in redirect URI.
...
- vite 8.3.1, @vitejs/plugin-react 6.1.1, typescript 7.0.2
- Node.js 22.23.3
...
```sh
npx degit algebrix/algebrix-identity-samples/spa/react my-spa
```
...
git sparse-checkout set spa/react
```
